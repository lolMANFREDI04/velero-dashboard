...
const keycloakLogoutUrl = `${process.env.KEYCLOAK_URL}/realms/${process.env.KEYCLOAK_REALM}/protocol/openid-connect/logout?redirect_uri=${encodeURIComponent(tools.subPath('/login'))}`;

app.use((req, res, next) => authController.globalSecureAction(req, res, next));

app.get('/auth/prova' ,()=> authController.provaapi())

app.get('/auth/keycloak', (req, res) => authController.keycloakLogin(req, res));
app.get('/auth/callback', (req, res) => authController.keycloakCallback(req, res));
